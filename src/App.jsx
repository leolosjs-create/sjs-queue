...
export default function App() {
  ...
  const KioskView = () => {
    ...
    useEffect(() => {
      // For Web Browsers (Chrome/Safari) - 3 second auto-close after print dialog
      if (printedTicket && !printLock.current && !useAndroidUSB) {
        printLock.current = true;
        const timer = setTimeout(() => {
          window.print();
          // After print dialog closes, wait 3 seconds then hide popup
          setTimeout(() => { 
            setPrintedTicket(null); 
            printLock.current = false;
          }, 3000); 
        }, 500); // Wait 0.5s for popup to render before blocking thread with print
    ...
    return (
      <>
        ...
        {/* STANDARD DESKTOP HTML PRINT LAYOUT */}
        {printedTicket && !useAndroidUSB && (
      ...
      return (
        <div className="h-[calc(100vh-64px)] bg-slate-900 flex flex-col items-center justify-center p-6 print:hidden">
          <div className="max-w-lg w-full bg-slate-800 p-12 rounded-3xl shadow-2xl text-center border-2 border-slate-700">
            ...
            <button onClick={() => setIsMonitorStarted(true)} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-8 rounded-3xl text-2xl shadow-[0_0_50px_rgba(37,99,235,0.4)] transition-all active:scale-95 flex items-center justify-center gap-4 tracking-widest uppercase">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <div className="w-full lg:w-1/3 bg-slate-800/40 p-8 md:p-12 flex flex-col">
            ...
            <div className="space-y-6 overflow-y-auto flex-1 pr-2">
              {waitingTickets.filter(t => new Date(t.createdAt).toDateString() === currentTime.toDateString()).sort((a,b) => new Date(a.createdAt) - new Date(b.createdAt)).slice(0, 5).map((ticket) => (
