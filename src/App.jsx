...
export default function App() {
  ...
  const KioskView = () => {
    ...
    const handlePrint = async (serviceId) => {
      ...
      if (ticket) {
        ...
        if (useAndroidUSB) {
          const rawbtUrl = `rawbt:base64,${btoa(`\x1B\x40\x1B\x61\x01${PHARMACY_NAME_ZH}\n${PHARMACY_NAME}\n\n${ticket.serviceNameZh}\n${ticket.serviceName}\n\nTicket Number:\n\x1D\x21\x11${ticket.ticketNumber}\x1D\x21\x00\n\n${formatDate(ticket.createdAt)}\n${formatTime(ticket.createdAt)}\n\n\n\n\n\x1DV\x00`)}`;
      ...
      <div className="min-h-[calc(100vh-64px)] bg-gray-100 flex flex-col items-center justify-center p-4 md:p-6 print:hidden">
        <div className="max-w-4xl w-full bg-white rounded-3xl shadow-2xl overflow-hidden relative flex flex-col max-h-full">
          <div className="bg-blue-900 p-6 md:p-8 text-center text-white flex flex-col items-center border-b-8 border-blue-600/30 shrink-0">
            ...
            <p className="text-blue-100 text-lg md:text-2xl font-medium mt-1">請選擇服務以領取籌號 Please select a service</p>
          </div>
          ...
          {printedTicket && (
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-sm text-center border-t-8 border-blue-600 animate-in zoom-in-95 duration-200">
                ...
                <div className="text-7xl md:text-8xl font-black text-blue-600 my-4 tracking-tighter">{printedTicket.ticketNumber}</div>
                <button onClick={() => window.print()} className="mt-2 bg-blue-50 text-blue-700 font-bold py-4 px-6 rounded-full flex items-center justify-center gap-2 hover:bg-blue-100 transition-colors w-full border border-blue-200 mb-6 text-xl"><Printer className="w-6 h-6" /> 列印籌號 Print</button>
        ...
        {printedTicket && (
          <div className="hidden print:block text-black text-center w-full max-w-[80mm] mx-auto p-4 font-sans bg-white z-[9999] m-0">
            <div className="flex flex-col items-center mb-4 border-b-2 border-black pb-4">
              <img src={LOGO_PATH} alt="Logo" className="w-11/12 max-w-[70mm] h-auto object-contain mb-4" />
            ...
            <div className="border-y-4 border-black py-6 my-4">
              ...
              <div className="text-[6.5rem] font-black leading-none">{printedTicket.ticketNumber}</div>
            </div>
  ...
  const MonitorView = () => {
    ...
    useEffect(() => {
      if (lastCallEvent.time && isMonitorStarted) {
        setFlash(true);
        const timer = setTimeout(() => setFlash(false), 3000);
        if (window.speechSynthesis) {
          window.speechSynthesis.cancel();
          setTimeout(() => {
            const tNumber = (currentTicket?.ticketNumber || displayId);
            const formattedTicket = tNumber.split('').join(' '); 
            const msgZh = new SpeechSynthesisUtterance(`請 ${formattedTicket} 號客, 到 ${currentTicket?.calledByCounter?.replace('Counter', '')?.replace('Room', '') || ''} 號${currentTicket?.calledByCounter?.includes('Counter') ? '櫃位' : '房間'}。`);
            msgZh.lang = 'zh-HK';
            msgZh.rate = 0.85;
            const msgEn = new SpeechSynthesisUtterance(`Ticket ${formattedTicket}, please proceed to ${currentTicket?.calledByCounter || ''}.`);
            msgEn.lang = 'en-US';
    ...
    }, [lastCallEvent.time, isMonitorStarted]);
      ...
      return (
        <div className="h-[calc(100vh-64px)] bg-slate-900 flex flex-col items-center justify-center p-6 print:hidden">
          <div className="max-w-lg w-full bg-slate-800 p-12 rounded-3xl shadow-2xl text-center border-2 border-slate-700">
            ...
            <h2 className="text-4xl font-bold text-white mb-4 tracking-tight">啟動叫號螢幕</h2>
            <p className="text-slate-400 mb-10 text-xl font-medium">點擊按鈕開啟聲音通知並開始運作。<br/><span className="text-sm opacity-50 uppercase mt-2 block tracking-widest">Enable sound & Start monitor</span></p>
            <button onClick={() => setIsMonitorStarted(true)} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-8 rounded-3xl text-2xl shadow-[0_0_50px_rgba(37,99,235,0.4)] transition-all active:scale-95 flex items-center justify-center gap-4 tracking-widest">
      ...
      <div className="min-h-[calc(100vh-64px)] bg-slate-900 text-white flex flex-col overflow-hidden print:hidden">
        <div className="w-full bg-slate-800 border-b-4 border-slate-700 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-8 shrink-0 z-20 shadow-2xl">
           <div className="flex items-center gap-6 md:gap-10">
              <div className="bg-white rounded-3xl p-3 md:p-4 shadow-inner ring-4 ring-slate-700/50">
                 <img src={LOGO_PATH} alt="Logo" className="h-16 md:h-24 lg:h-32 object-contain" onError={(e) => e.target.style.display='none'} />
              </div>
           ...
           <div className="flex flex-col items-center md:items-end shrink-0">
              <h1 className="text-5xl md:text-7xl font-black text-yellow-500 tracking-[0.2em] uppercase mb-1 drop-shadow-lg">現在叫號</h1>
              <p className="text-slate-500 text-sm md:text-xl font-black tracking-[0.3em] uppercase opacity-40">Now Calling</p>
           </div>
        </div>
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          <div className="w-full lg:w-2/3 p-8 flex flex-col justify-center items-center border-b lg:border-b-0 lg:border-r-4 border-slate-700/50">
            <div className={`transition-all duration-300 text-center ${flash ? 'scale-110 text-white drop-shadow-[0_0_80px_rgba(255,255,255,0.6)]' : 'text-white'}`}>
              <div className="text-[14rem] md:text-[24rem] lg:text-[28rem] font-black leading-none my-4 tracking-tighter drop-shadow-2xl">{displayId}</div>
            </div>
            {currentTicket && currentTicket.calledByCounter && (
              <div className="text-center animate-in fade-in slide-in-from-bottom-8 mt-10">
                <div className="inline-block bg-yellow-500 text-slate-900 px-16 py-5 md:px-24 md:py-8 rounded-full text-4xl md:text-7xl font-black shadow-[0_0_50px_rgba(234,179,8,0.4)] border-4 border-white/20">
            ...
            <div className="space-y-6 overflow-y-auto flex-1 pr-2">
              {tickets.filter(t => t.status === 'waiting' && new Date(t.createdAt).toDateString() === currentTime.toDateString()).sort((a,b) => new Date(a.createdAt) - new Date(b.createdAt)).slice(0, 5).map((ticket) => (
                <div key={ticket.id} className="flex justify-between items-center bg-slate-700/40 p-8 md:p-10 rounded-[2.5rem] border-2 border-slate-600/30 shadow-2xl">
                  <span className="text-6xl md:text-8xl font-black text-slate-100 tracking-tighter">{ticket.ticketNumber}</span>
  ...
  const PanelView = () => {
    ...
    if (!panelRoom) {
      return (
        <div className="h-[calc(100vh-64px)] bg-gray-100 flex items-center justify-center p-4 print:hidden">
      ...
      <div className="h-auto lg:h-[calc(100vh-64px)] bg-gray-100 p-4 lg:p-6 flex flex-col lg:flex-row gap-4 lg:gap-6 overflow-y-auto lg:overflow-hidden print:hidden">
        <div className="flex-1 flex flex-col gap-4 lg:gap-6 lg:overflow-hidden">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 lg:p-6 shrink-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 lg:gap-3">
              {SERVICES.map(service => {
                const next = tickets.filter(t => t.status === 'waiting').sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt)).find(t=>t.type===service.id);
                return (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex-1 min-h-[300px] lg:min-h-0 flex flex-col overflow-hidden">
            ...
            <div className="p-4 overflow-y-auto space-y-3 lg:space-y-4 flex-1">
              {tickets.filter(t => ['calling', 'arrived'].includes(t.status) && t.calledByCounter === panelRoom).map(ticket => {
                ...
                return (
                  <div key={ticket.id} className={`p-4 rounded-xl border-l-4 shadow-sm flex flex-col xl:flex-row gap-4 xl:gap-0 justify-between items-start xl:items-center ${ticket.status === 'calling' ? 'bg-yellow-50 border-yellow-400' : 'bg-green-50 border-green-500'}`}>
                    <div>
                      <div className="text-3xl font-black text-gray-900">{ticket.ticketNumber}</div>
                    ...
                    <div className="flex gap-2 w-full xl:w-auto">
                      <button onClick={() => setMemoModal({ id: ticket.id, displayId: ticket.ticketNumber, text: ticket.memo || '' })} className="p-3 bg-white rounded-lg border border-gray-200 text-gray-500 hover:text-blue-600"><Edit3 className="w-5 h-5" /></button>
                      <button onClick={() => setReturnModal({ id: ticket.id, displayId: ticket.ticketNumber })} className="p-3 bg-white rounded-lg border border-gray-200 text-orange-500"><RotateCcw className="w-5 h-5" /></button>
                      {ticket.status === 'calling' ? (
                        <>
                          ...
                          <button onClick={()=>updateTicketStatus(ticket.id, 'missed')} className="bg-red-50 text-red-600 font-bold px-4 py-3 rounded-lg">過號 Miss</button>
                        </>
                      ) : (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col shrink-0 overflow-hidden transition-all duration-300">
            ...
            {isHistoryExpanded && (
              <div className="p-4 border-t border-gray-200 max-h-80 overflow-y-auto space-y-2 bg-white">
                {tickets.filter(t => ['completed', 'missed', 'cancelled'].includes(t.status) && new Date(t.createdAt).toDateString() === currentTime.toDateString()).sort((a, b) => new Date(b.completedAt || 0) - new Date(a.completedAt || 0)).slice(0, 50).map(t => (
                  <div key={t.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 gap-2 sm:gap-4">
                    <div className="flex items-center gap-3 shrink-0">
                      <span className={`text-xl font-black ${t.status === 'completed' ? 'text-green-600' : 'text-red-500'}`}>{t.ticketNumber}</span>
        ...
        <div className="w-full lg:w-96 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col shrink-0">
          <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col gap-3 shrink-0">
            <div className="flex justify-between items-center"><h3 className="font-bold text-gray-700 flex items-center gap-2"><Users className="w-5 h-5 text-blue-500"/> 等待隊列 Queue <span className="bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full font-bold">{tickets.filter(t => t.status === 'waiting').length}</span></h3></div>
            <div className="flex bg-gray-200 p-1 rounded-lg">
              <button onClick={() => setQueueSortBy('time')} className={`flex-1 flex justify-center items-center gap-1 text-xs font-bold py-2 rounded-md transition-colors ${queueSortBy === 'time' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-500'}`}><Timer className="w-4 h-4" /> 時間 Time</button>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
            {tickets.filter(t => t.status === 'waiting').sort((a,b) => {
              if (queueSortBy === 'number') return (a.ticketNumber).localeCompare(b.ticketNumber);
              return new Date(a.createdAt) - new Date(b.createdAt);
            }).map(t => (
              <div key={t.id} onContextMenu={(e) => { e.preventDefault(); setMemoModal({ id: t.id, displayId: t.ticketNumber, text: t.memo || '' }); }} className="p-4 transition-colors flex justify-between items-center group relative border-l-4 hover:bg-gray-50">
                <div className="flex-1 min-w-0 pr-4">
                  <div className="text-xl font-black">{t.ticketNumber}</div>
                  <div className="text-xs font-bold mt-1 text-gray-600">已等待: {getWaitTimeMinutes(t.createdAt, currentTime)} 分鐘</div>
                  {t.memo && <div className="text-xs mt-2 p-1.5 rounded border border-blue-100 bg-blue-50 text-blue-700 truncate font-bold">{t.memo}</div>}
                </div>
                <div className="flex gap-2 shrink-0 items-center">
                  ...
                  <button onClick={() => setMemoModal({ id: t.id, displayId: t.ticketNumber, text: t.memo || '' })} className="p-2 text-gray-400 hover:text-blue-600 transition-colors shadow-sm bg-white rounded border border-gray-100"><Edit3 className="w-4 h-4"/></button>
                  <button onClick={() => setDeleteModal({ id: t.id, displayId: t.ticketNumber })} className="p-2 text-red-300 hover:text-red-600 transition-colors shadow-sm bg-white rounded border border-gray-100"><Trash2 className="w-4 h-4"/></button>
                </div>
              </div>
            ))}
          </div>
